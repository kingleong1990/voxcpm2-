...
class VoxCPMDemo:
    def __init__(self, model_dir: Optional[str] = None) -> None:
        ...
        # Check if SenseVoiceSmall model exists locally first
        local_asr_model_path = os.path.join("models", "SenseVoiceSmall")
        if os.path.isdir(local_asr_model_path):
            logger.info(f"Loading ASR model from local path: {local_asr_model_path}")
            self.asr_model: Optional[AutoModel] = AutoModel(
                model=local_asr_model_path,
                disable_update=True,
                log_level="DEBUG",
                device="cuda:0" if self.device == "cuda" else "cpu",
            )
        else:
            logger.info(f"Loading ASR model from remote repository: {self.asr_model_id}")
            self.asr_model: Optional[AutoModel] = AutoModel(
    ...
    def _resolve_model_dir(self) -> str:
        if self.explicit_model_dir and os.path.isdir(self.explicit_model_dir):
            logger.info(f"Using explicit model directory: {self.explicit_model_dir}")
        ...
        if env_model_dir and os.path.isdir(env_model_dir):
            logger.info(f"Using VOXCPM_MODEL_DIR: {env_model_dir}")
            return env_model_dir
        
        # Check if the VoxCPM2 model exists in the local models folder
        local_voxcpm_path = os.path.join("models", "openbmb__VoxCPM2")
        if os.path.isdir(local_voxcpm_path):
            logger.info(f"Using local model: {local_voxcpm_path}")
            return local_voxcpm_path
        
        # Check alternative local path (VoxCPM2.0 folder)
        alt_local_path = os.path.join("VoxCPM2.0")
        if os.path.isdir(alt_local_path):
            logger.info(f"Using alternative local model path: {alt_local_path}")
        ...
        if len(repo_id) > 0:
            ...
            if not os.path.isdir(target_dir):
                try:
                    ...
                    logger.info(f"Model downloaded successfully to: {target_dir}")
                except Exception as e:
                    logger.error(f"HF download failed: {e}")
                    logger.error("Please check your internet connection or download the model manually.")
                    raise RuntimeError(f"Model not found and download failed: {e}")
            return target_dir
        
        # If we reach here, no model was found
        raise RuntimeError(
            f"Model not found! Please download the model and place it in one of these locations:\n"
            f"  1. {local_voxcpm_path}\n"
            f"  2. {alt_local_path}\n"
            f"  3. Or set VOXCPM_MODEL_DIR environment variable"
        )
    ...
    def get_or_load_voxcpm(self) -> voxcpm.VoxCPM:
        ...
        logger.info("This may take 1-3 minutes on first run, please wait...")
        logger.info("=" * 50)
        try:
...
def run_demo(
    server_name: str = "0.0.0.0",  # 修改为允许所有IP访问，而不是仅限本地
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Server host")  # 修改为0.0.0.0，允许所有IP访问
    args = parser.parse_args()
    run_demo(model_dir=args.model_dir, server_port=args.port, server_name=args.host)
